...
export default function HomeScreen() {
  ...
  // Use the loading state hook to track all async operations
  const { isLoading, trackPromise } = useLoadingState(true);

  // Fetch characters and read status when component mounts or user changes
  useEffect(() => {
    if (user) {
      // Track all promises to ensure loading state is accurate
      const loadData = async () => {
        try {
          await Promise.all([
            trackPromise(fetchCharacters()),
            trackPromise(fetchCampaignReadStatus()),
            trackPromise(fetchCampaignInvitations()),
            trackPromise(fetchFriendRequestsReceived())
          ]);
        } catch (error) {
  ...
  }, [user, fetchCharacters, fetchCampaignReadStatus, fetchCampaignInvitations, fetchFriendRequestsReceived, initializeReadStatusRealtime, trackPromise]);
